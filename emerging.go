...
func main() {
	...
	max_reduce_kill := make(chan int)
	go reducer(max_word, emap, "INVALID", 0, max_reduce_kill)
	min_reduce_kill := make(chan int)
	go reducer(min_word, emap, "INVALID", 2147483647, min_reduce_kill)
	fmt.Println("\nRunning\n")
	wg.Wait()
